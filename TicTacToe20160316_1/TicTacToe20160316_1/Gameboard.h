...
public:
	...
	int moveCnt;			/* 현재 수 개수 */
	int	state;				/* 현재 게임판 상태 */
	int	Level;			/* 게임을 시작한 컴퓨터 레벨 */
	char playerCom;		/* 게임을 시작한 컴퓨터 종류 */
	char oppnentCom;		/* 상대방 컴퓨터 종류 */

	static const int STATE_WINC = 3;		/* 컴퓨터가 이겼을 때 */
	static const int STATE_WINP = 4;		/* 사람이 이겼을 때 */
...
public:
	...
	void InitBoard(int startCom, int movedCnt, int nlevelC);	/* 게임판 초기화 */

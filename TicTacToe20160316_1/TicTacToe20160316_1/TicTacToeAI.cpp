...
int GameAI::GetPossibleMove(Position * iList)
{
	...
	GameBoard tempBoard = ai_board;	/* 게임판 복사 */
		...
		for (int j = 0; j<4; j++)
		{
			if (ai_board.board[i][j] == ' ')
			{
				...
				for (int k = 0; k<iNum; k++)
				{
					...
					check = CheckSymmetric(tempBoard, ai_board);	/* 대칭성 검사 */
					tempBoard.UndoMove();
					ai_board.UndoMove();
					if (check == 1)		/* 대칭된다면 스킵 */
						break;
